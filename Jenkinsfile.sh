...
echo "  OK  flower-api pod $POD loaded pgOrders module (write=RPi8, read=xsyysj,MacMini)"
POD_LA=$(kubectl -n new-ecommerce get pod --no-headers -l app=flower-api-la | awk '$3=="Running"{print $1; exit}')
if [ -n "$POD_LA" ]; then
  if kubectl -n new-ecommerce logs "$POD_LA" --tail=200 2>&1 | grep -q '\[pgOrders\]'; then
    echo "  OK  flower-api-la pod $POD_LA loaded pgOrders module"
  else
    echo "  WARN flower-api-la pod $POD_LA missing [pgOrders] init log"
  fi
fi
