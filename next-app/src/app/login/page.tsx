...
type SP = { redirect?: string; error?: string; callbackUrl?: string; mode?: 'phone' | 'email'; register?: string };
...
export default async function LoginPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  const err = sp.error || '';
  const registerOk = sp.register === 'ok';
    ...
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{
      ...
      >
        ...
        <div style={{
          ...
          <a
            href={`/login?mode=phone&redirect=${encodeURIComponent(redirect)}${registerOk ? '&register=ok' : ''}`}
            style={{
          ...
          >📱 {t('auth.tabPhone')}</a>
          <a
            href={`/login?mode=email&redirect=${encodeURIComponent(redirect)}${registerOk ? '&register=ok' : ''}`}
            style={{
              textAlign: 'center', padding: '8px 10px', borderRadius: 8, fontSize: 13, fontWeight: 700,
              textDecoration: 'none',
              background: !phoneActive ? '#ffffff' : 'transparent',
