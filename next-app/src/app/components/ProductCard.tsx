...
export default function ProductCard({ product, compact = false }: Props) {
  ...
  const shippingText = product.shipping_description || (product.shippingFee === 0 ? '免运费' : product.shippingFee ? `运费 ${formatPrice(product.shippingFee, region.code)}` : '');
    ...
    return (
      <div className="flex items-center gap-3 bg-white rounded-xl p-3 shadow-sm">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-sm font-bold text-red-600 mt-0.5">{formatPrice(price, region.code)}</p>
          {shippingText && (
    ...
    <div className="flex-shrink-0 w-36 bg-white rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="p-2">
        ...
        <div className="flex items-center justify-between mt-1">
          <span className="text-sm font-bold text-red-600">{formatPrice(price, region.code)}</span>
          {product.distance != null && (
