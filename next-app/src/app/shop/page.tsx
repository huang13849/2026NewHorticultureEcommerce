...
export default function ShopPage() {
  ...
  const addToCart = (product: Product) => {
    ...
    const name = product.title || product.flowerName || '未命名';
  ...
  return (
    <main className="min-h-screen bg-white text-stone-900 pb-20">
      ...
      <div className="sticky top-0 z-10 bg-white/80 backdrop-blur-xl border-b border-stone-200/60 px-6 py-4">
        <h1 className="text-lg font-bold text-center">花卉商城</h1>
      </div>
      ...
      <div className="px-4 py-4">
        {loading ? (
          ...
          <div className="text-center py-16 text-stone-400">暂无商品</div>
        ) : (
          <div className="grid grid-cols-2 gap-3">
            {filtered.map(p => {
              const img = getImg(p);
              const price = Number(p.sellPrice || p.price || p.settlementPrice || 0);
              const name = p.title || p.flowerName || '未命名';
                ...
                <div key={p._id} className="rounded-2xl border border-stone-200 bg-white overflow-hidden hover:border-emerald-300 hover:shadow-md transition-all">
                  ...
                  <div className="p-2.5">
                    ...
                    <div className="flex items-center justify-between mt-1.5">
                      ...
                      <button
                        onClick={() => addToCart(p)}
                        className="bg-emerald-700 text-white text-[10px] px-2.5 py-1 rounded-lg hover:bg-emerald-800 transition-colors"
                      >
                        + 加入
                      </button>
                    </div>
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-14 left-0 right-0 bg-white border-t border-stone-200 px-6 py-3 z-10">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-xs text-stone-500">🛒 {totalItems}件 </span>
              <span className="text-lg font-bold text-emerald-700">¥{totalAmount.toFixed(2)}</span>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setCart([])} className="text-xs text-stone-400 px-3 py-2">清空</button>
